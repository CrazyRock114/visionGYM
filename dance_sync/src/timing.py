...
headers arriving. Both are approximations — the kernel buffers, so a small body
can look like it uploaded instantly — but on a multi-megabyte video the split is
meaningful, and it is the difference between blaming the model and blaming the
network.
"""
