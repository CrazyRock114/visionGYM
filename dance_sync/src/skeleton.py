...
# (a, b, side); side picks the color under COLOR_BY = "side". "H" edges all
# touch a face keypoint, so dropping FACE_KPTS removes them together with the
# ear-to-shoulder lines.
SKELETON = [
...
def subject_color(person: dict) -> tuple[int, int, int]:
    """The color for one subject.

    Keyed on the ``slot`` src.pose.clean() stamps on each survivor — its rank in
    order of first appearance — not on ``track_id``. Track ids are arbitrary
    (this clip's three dancers come back as 0, 1 and 4), and indexing the
    palette with them gave two of the three nearly the same blue.
    """
    key = person.get("slot", person.get("track_id"))
    if key is None:
        return (200, 200, 200)
    return TRACK_COLORS[int(key) % len(TRACK_COLORS)]
...
def draw_person(
    ...
    *color_by* decides what the limb color means. ``"track"`` gives each subject
    one color, which is what tells three dancers apart when their skeletons
    cross; ``"side"`` colors left/right/torso instead, which reads better on a
    single subject but makes a crowd uniform.
    """
    ...
    if draw_bbox:
        ...
        if draw_label:
            ...
            cv2.putText(img, label, (p0[0], max(14, p0[1] - 6)),
                        ...
                        tint if color_by == "track" else bbox_color, 2, cv2.LINE_AA)

    for a, b, side in edges:
        if ok(a) and ok(b):
            color = tint if color_by == "track" else SIDE_COLORS[side]
